...
const writeDiagnosticsDirectory = (diagnostics: Diagnostic[]): string => {
  const outputDirectory = join(tmpdir(), `htmx-doctor-${randomUUID()}`);
...
const printBranding = (score?: number): void => {
  ...
  logger.log(`  HTMX Doctor ${highlighter.dim("(www.htmx.doctor)")}`);
  logger.break();
};
...
const buildBrandingLines = (
  ...
  if (scoreResult) {
    ...
    lines.push(
      createFramedLine(
        "HTMX Doctor (www.htmx.doctor)",
        `HTMX Doctor ${highlighter.dim("(www.htmx.doctor)")}`,
      ),
    );
...
const printProjectDetection = (
  ...
  const htmxLabel = projectInfo.htmxVersion
    ? `HTMX ${projectInfo.htmxVersion}`
    : `${projectInfo.htmxSourceFileCount} HTMX source files`;
  ...
  if (projectInfo.framework === "unknown") {
    completeStep("Detecting framework. No framework-specific signature found.");
  } else {
    completeStep(`Detecting framework. Found ${highlighter.info(frameworkLabel)}.`);
  }
  completeStep(`Detecting HTMX usage. Found ${highlighter.info(htmxLabel)}.`);
  completeStep(`Detecting language. ${highlighter.info(languageLabel)}.`);
  ...
  if (userConfig) {
    completeStep(`Loaded ${highlighter.info("htmx-doctor config")}.`);
  }
...
export const scan = async (
  ...
  const lintPromise = options.lint
    ? (async () => {
        const lintSpinner = options.scoreOnly ? null : spinner("Running HTMX checks...").start();
        try {
          const lintDiagnostics = await runOxlint(directory, sourceIncludePaths);
          lintSpinner?.succeed("Running HTMX checks.");
          return lintDiagnostics;
        } catch (error) {
          didLintFail = true;
          if (!options.scoreOnly) {
            lintSpinner?.fail("HTMX checks failed (non-fatal, skipping).");
  ...
  const deadCodePromise =
    options.deadCode && !isDiffMode && projectInfo.hasPackageJson
      ? (async () => {
