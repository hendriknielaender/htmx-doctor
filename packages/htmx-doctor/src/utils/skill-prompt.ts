...
const CONFIG_DIRECTORY = join(HOME_DIRECTORY, ".htmx-doctor");
const CONFIG_FILE = join(CONFIG_DIRECTORY, "config.json");

const SKILL_NAME = "htmx-doctor";
const WINDSURF_MARKER = "# HTMX Doctor";

const SKILL_DESCRIPTION =
  "Run after making HTMX changes to catch request, swap, and markup issues early.";

const SKILL_BODY = `Scans HTMX templates and scripts for security, performance, correctness, and accessibility issues. Outputs a 0-100 score with actionable diagnostics.
...
const AGENTS_CONTENT = `# HTMX Doctor
...
const installSkill = (): void => {
  ...
  if (installedCount === 0) {
    ...
    logger.success("Done! The skill will activate when working on HTMX projects.");
  }
};
...
export const maybePromptSkillInstall = async (shouldSkipPrompts: boolean): Promise<void> => {
  ...
  logger.dim(
    `   Install the ${highlighter.info("htmx-doctor")} skill to teach Cursor, Claude Code,`,
  );
  logger.dim("   Ami, and other AI agents how to diagnose and fix HTMX issues.");
  logger.break();
