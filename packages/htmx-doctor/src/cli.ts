...
const exitWithFixHint = () => {
  ...
  logger.dim("Run `npx htmx-doctor@latest --fix` to fix issues.");
  logger.break();
  process.exit(0);
};
...
const program = new Command()
  .name("htmx-doctor")
  .description("Diagnose HTMX codebase health")
  ...
  .action(async (directory: string, flags: CliFlags) => {
    ...
    try {
      ...
      if (!isScoreOnly) {
        logger.log(`htmx-doctor v${VERSION}`);
        logger.break();
      }
...
${highlighter.dim("Learn more:")}
  ${highlighter.info("https://github.com/millionco/htmx-doctor")}
`,
  );

const DEEPLINK_FIX_PROMPT = "/{slash-command:ami:htmx-doctor}";
...
const FIX_COMMAND_HINT = "npx htmx-doctor@latest --fix";
...
const buildAmiBanner = (
  ...
  return renderFramedBoxString([
    ...
    createFramedLine(""),
    createFramedLine(
      `Ami is a coding agent built for HTMX workflows. It reads`,
      `${highlighter.info("Ami")} is a coding agent built for HTMX workflows. It reads`,
    ),
    createFramedLine("your htmx-doctor report, understands your codebase,"),
    createFramedLine(
    ...
    choices: [
      {
        ...
        description: "Optimized coding agent for HTMX Doctor",
...
const fixCommand = new Command("fix")
  .description("Open Ami to auto-fix htmx-doctor issues")
